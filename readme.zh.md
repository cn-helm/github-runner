...
- `readme.md` / `readme.zh.md`：英文和简体中文文档。
- `docs/render.py`：将两份 README 渲染为 GitHub Pages 静态文档。
...
| `github.repo`                                 | `org/repo`，仅支持 github.com 仓库级 runner                         |
...
1. 确认无正在执行的 job，暂停新任务，执行
   `kubectl scale deployment/${repo}-runner-runner -n github-runner --replicas=0`，等待原 Pod 完全退出。
2. 备份 PVC。使用同一个 release 和原覆盖文件启用维护模式：
   `helm upgrade ${repo}-runner . -n github-runner -f runner.local.yaml --set runner.maintenance=true`。
   Chart 恢复一个 Pod，准备依赖后只执行 sleep，不连接 GitHub。
   使用 `kubectl exec -it -n github-runner deployment/${repo}-runner-runner -c runner -- setpriv --reuid=1001 --regid=1001 --init-groups --no-new-privs /bin/bash`
   进入非 root 维护终端。
...
4. 准备有效的**注册 token**，更新配置，执行
   `helm upgrade ${repo}-runner . -n github-runner -f runner.local.yaml --set runner.maintenance=false`
   退出维护模式并恢复 runner。
...
默认 PVC `${repo}-runner-runner` 保留，其中含注册凭据、源码及缓存，应按凭据数据管理访问和备份。
重新安装时显式指定 `--set persistence.existingClaim=${repo}-runner-runner`，并使用原 runner 名称及设置。
...
推送到 `main` 且修改 Chart、values、schema、Customize 表单、脚本、模板、任一 README 或文档工具时自动触发，
也可在 GitHub Actions 页面手动运行 **Publish Helm repository**。
流程先执行 `helm lint . --strict`，再打包 Chart、合并现有索引、渲染文档，并发布到 `gh-pages` 分支。
...
站点首页 `index.html` 展示英文 README，`readme.zh.html` 展示中文版，两页均提供语言切换链接。
原始 `readme.md` 和 `readme.zh.md` 也会发布供下载；Helm 客户端继续通过同一仓库地址访问 `index.yaml` 和 Chart 安装包。
workflow 使用固定版本的 Python Markdown 生成静态 HTML，无需 Jekyll 构建。
本地预览可在临时 Python 虚拟环境中安装 `docs/requirements.txt`，再执行 `python docs/render.py --output .helm-repository`。
